...
class Vector(object):
    """
    Class representing a vector. Used to determine position of the turtle
    as well as heading.
...
class Turtle(object):
    ...
    other implementations. Note that currently each turtle has exactly
    ONE style of drawing, so if you intend to draw in multiple styles several
    instances of turtles are needed.
    A turtle will only actually draw when the pen is down (default=false).
    An xml representation usable for pysvg can ge retrieved using the
    getXML()-method.
    To add the turtles paths to an svg you have two opions:
    Either you simply call "addTurtlePathToSVG" or
    you can create an svg element and
    append the Elements of the turtle using a loop, e.g:
    s=svg(...)
    t=Turtle(...)
    for element in t.getSVGElements():
        s.addElement(element)
    """
    def __init__(self, initialPosition=Vector(0.0, 0.0),
                 initialOrientation=Vector(1.0, 0.0), fill='white',
                 stroke='black', strokeWidth='1', penDown=False):
        """
        Initializes a new Turtle with a new initial position and
        orientation as well as defaultvalues for style.
        """
        self.fill = fill
        self.stroke = stroke
    ...
    def finish(self):
        """
        MUST be called when drawing is finished.
        Else the last path will not be added to the stack.
        """
        self._addPolylineToElements()
    ...
    def _addPolylineToElements(self):
        """
        Creates a new Polyline element that will be used for future
        movement/drawing. The old one (if filled) will be stored on the
        movement stack.
        """
        if (len(self._pointsOfPolyline) > 1):
            ...
            for point in self._pointsOfPolyline:
                s += str(point) + ' '
            p = Polyline(s)
            p.set_style('fill:' + self.fill + '; stroke:' + self.stroke +
                        '; stroke-width:' + self.strokeWidth)
            self._svgElements.append(p)
        self._pointsOfPolyline = []
        self._pointsOfPolyline.append(Vector(self._position.x,
                                             self._position.y))

    def getXML(self):
        """
        Retrieves the pysvg elements that make up the turtles path and returns
        them as String in an xml representation.
        """
        s = ''
    ...
    def getSVGElements(self):
        """
        Retrieves the pysvg elements that make up the turtles path
