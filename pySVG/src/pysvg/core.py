...
from attributes import CoreAttrib, ConditionalAttrib, StyleAttrib,\
                       GraphicalEventsAttrib, PaintAttrib, OpacityAttrib,\
                       GraphicsAttrib, CursorAttrib, FilterAttrib, MaskAttrib,\
                       ClipAttrib
import codecs
...
class BaseElement:
    """
    This is the base class for all svg elements like title etc.
    ...
    def getAllElements(self):
        """ returns all elements contained within the top level element
    ...
    def getElementByID(self, anID):
        """ returns an element with the specific id and the position of that
         element within the svg elements array
        """
        pos = 0
        for element in self._subElements:
            if element.get_id() == anID:
                return (element, pos)
            pos += 1

    def getElementsByType(self, aType):
        """
        retrieves all Elements that are of type type
        @type  aType: class
        @param aType: type of the element
        """
        foundElements = []
        for element in self.getAllElementsOfHirarchy():
            if isinstance(element, aType):
    ...
    def getXML(self):
        ...
        It is also used by getXML in SVG
    ...
    # generic methods to set and get atributes
    # (should only be used if something is not supported yet
    ...
    def save(self, filename, encoding='ISO-8859-1', standalone='no'):
        ...
        Calling this method only makes sense if the root element is an
         svg elemnt
        """
...
class BaseShape(BaseElement, CoreAttrib, ConditionalAttrib, StyleAttrib,
                GraphicalEventsAttrib, PaintAttrib, OpacityAttrib,
                GraphicsAttrib, CursorAttrib, FilterAttrib, MaskAttrib,
                ClipAttrib):
    """
    Baseclass for all shapes. Do not use this class directly.
     There is no svg element for it
    """
