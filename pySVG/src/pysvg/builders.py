...
from shape import Circle, Rect, Ellipse, Polygon, Polyline, Line
...
class ShapeBuilder:
    """
    Helper class that creates commonly used objects and shapes with predefined
     styles and few but often used parameters. Used to avoid more
      complex coding for common tasks.
    """
    ...
    def createEllipse(self, cx, cy, rx, ry, strokewidth=1,
                      stroke='black', fill='none'):
        """
        Creates an ellipse
        @type  cx: string or int
        ...
        @param stroke:  color with which to draw the outer limits
        @type  fill: string (either css constants like "black" or
         numerical values like "#FFFFFF")
        @param fill:  color with which to fill the element(default: no filling)
    ...
    def createRect(self, x, y, width, height, rx=None, ry=None, strokewidth=1,
                   stroke='black', fill='none'):
        """
        Creates a Rectangle
        ...
        @type  rx: string or int
        @param rx:  For rounded rectangles, the x-axis radius of the ellipse
         used to round off the corners of the rectangle.
        @type  ry: string or int
        @param ry:  For rounded rectangles, the y-axis radius of the ellipse
         used to round off the corners of the rectangle.
        @type  strokewidth: string or int
        @param strokewidth:  width of the pen used to draw
        @type  stroke: string (either css constants like "black" or numerical
    ...
    def createPolygon(self, points, strokewidth=1,
                      stroke='black', fill='none'):
        """
        Creates a Polygon
    ...
    def createPolyline(self, points, strokewidth=1, stroke='black'):
        ...
        """
        style_dict = {'fill': 'none', 'stroke-width': strokewidth,
                      'stroke': stroke}
        myStyle = StyleBuilder(style_dict)
    ...
    def convertTupleArrayToPoints(self, arrayOfPointTuples):
        ...
        @type  arrayOfPointTuples: An array containing tuples
         eg.[(x1,y1),(x2,y2]
        @param arrayOfPointTuples:  All points needed to create the shape
        @return a string in the form "x1,y1 x2,y2 x3,y3"
        """
        points = ""
        for pointtuple in arrayOfPointTuples:
            points += str(pointtuple[0]) + "," + str(pointtuple[1]) + " "
        return points
...
class StyleBuilder:
    """
    Class to create a style string for those not familiar with svg
     attribute names.
    How to use it:
    ...
    # is used to provide a potential indirect value (currentColor)
    # for the 'fill', 'stroke', 'stop-color' properties.
...
class TransformBuilder:
    ...
    def getTransform(self):
        string = ''  # style="'
        for k, value in self.transform_dict.items():
            if value is not None and value != '':
                # string+=str(k)+':'+str(value)+'; '
                string += str(value) + ' '
        return string
